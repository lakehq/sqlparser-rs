...
impl<'a> Parser<'a> {
    ...
    pub fn parse_interval(&mut self) -> Result<Expr, ParserError> {
        ...
        loop {
            let item = self.maybe_parse(|parser| {
                if !values.is_empty() && parser.parse_keyword(Keyword::TO) {
                    ...
                    return parser_err!("Unexpected TO", parser.peek_token().span.start);
                }
                ...
                let value = if parser.dialect.require_interval_qualifier() {
        ...
        if values.is_empty() {
            parser_err!(
                "INTERVAL must have at least one valid value along with a unit if required",
    ...
    pub fn parse_value(&mut self) -> Result<Value, ParserError> {
        ...
        match next_token.token {
            ...
            Token::Number(n, p) => Ok(Value::Number(Self::parse(n, span.start)?, p)),
    ...
    fn parse_data_type_helper(
        ...
        let mut data = match &next_token.token {
            Token::Word(w) => match w.keyword {
                ...
                Keyword::INTERVAL => {
                    let unit = self
                        .maybe_parse(|parser| parser.parse_interval_unit())?
                        .unwrap_or(IntervalUnit {
                            leading_field: None,
                            leading_precision: None,
                            last_field: None,
                            fractional_seconds_precision: None,
                                ...
                                if s.ends_with('.') {
                                    let Some(s) = s.split('.').next().filter(|s| {
                                        ...
                                        return self.expected(
                                            ...
                                            TokenWithSpan::new(Token::Number(s, None), token.span),
                                        );
                                    };
