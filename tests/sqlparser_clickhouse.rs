            ...
            assert_eq!(
                ...
                Some(vec![
                    Setting {
                        ...
                        value: Number("1".parse().unwrap(), None)
                    },
                    Setting {
                        key: Ident::new("max_block_size"),
                        value: Number("10000".parse().unwrap(), None)
                    },
                ])
            );
        }
        ...
        Statement::Query(query) => {
            ...
            assert_eq!(
                ...
                Some(&BinaryOp {
                    ...
                    op: BinaryOperator::Eq,
                    right: Box::new(Expr::Value(Value::Number("2".parse().unwrap(), None))),
                })
            );
        }
        _ => unreachable!(),
    ...
    check_settings(
        ...
        &TableFunctionArgs {
            ...
            settings: Some(vec![
                Setting {
                    key: "s0".into(),
                    value: Value::Number("3".parse().unwrap(), None),
                },
                Setting {
                    key: "s1".into(),
